...
console.log('Sostituzione semplice e diretta delle date...');
...
// Sostituisce TUTTE le occorrenze di "Data in caricamento..." con le date reali
...
// Sostituisce tutte le occorrenze di "Data in caricamento..." con le date reali
// Usa un approccio più semplice: sostituisce tutto e poi ricostruisce
const datePattern = /<span class="news-date">Data in caricamento\.\.\.<\/span>/g;

// Prima sostituisce con placeholder temporanei
updatedContent = updatedContent.replace(datePattern, '<span class="news-date">TEMP_DATE_PLACEHOLDER</span>');

// Poi sostituisce i placeholder con le date reali in base al filename
Object.keys(filenameToDate).forEach(filename => {
    ...
    const articlePattern = new RegExp(
        `(<a[^>]*href="[^"]*${filename.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}[^"]*"[^>]*>.*?</a>.*?<div[^>]*class="news-meta"[^>]*>.*?<span class="news-date">)TEMP_DATE_PLACEHOLDER(</span>.*?</div>.*?</article>)`,
        's'
    );
...
// Se ci sono ancora placeholder, li sostituisce con una data generica
updatedContent = updatedContent.replace(/<span class="news-date">TEMP_DATE_PLACEHOLDER<\/span>/g, '<span class="news-date">Data non disponibile</span>');

// Conta quante date sono state sostituite
const replacedCount = (newsContent.match(/Data in caricamento\.\.\./g) || []).length;
const finalCount = (updatedContent.match(/Data in caricamento\.\.\./g) || []).length;
...
console.log('✅ Date sostituite direttamente!');
console.log('📅 Ora le date dovrebbero essere visibili immediatamente');
