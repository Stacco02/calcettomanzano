...
console.log('Sostituzione delle date specifiche per ogni singolo articolo...');
...
articlesWithDates.forEach(article => {
    const date = article.formattedDate;
    const filename = article.filename;
    
    // Cerca l'articolo specifico e sostituisce la sua data
    const articlePattern = new RegExp(
        `(<a[^>]*href="[^"]*${filename.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}[^"]*"[^>]*>.*?</a>.*?<div[^>]*class="news-meta"[^>]*>.*?<span class="news-date">)14 giugno 2025(</span>.*?</div>.*?</article>)`,
        's'
    );
...
console.log(`\nDate sostituite: ${replacedCount}`);

// Salva la pagina aggiornata
fs.writeFileSync(newsPagePath, updatedContent);

console.log('✅ Date specifiche sostituite per ogni articolo!');
console.log('📅 Ora ogni articolo ha la sua data corretta nella preview');
