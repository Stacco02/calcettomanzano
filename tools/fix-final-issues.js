...
console.log('Correggendo definitivamente date e filtro...');
...
// Sostituisce direttamente le date nell'HTML
let updatedContent = newsContent;

// Sostituisce "Data in caricamento..." con le date reali
articlesWithDates.forEach(article => {
    ...
    const articlePattern = new RegExp(
        `(<article[^>]*>.*?<a[^>]*href="[^"]*${article.filename.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}[^"]*"[^>]*>.*?</a>.*?<div[^>]*class="news-meta"[^>]*>.*?<span class="news-date">)Data in caricamento\\.\\.\\.(</span>.*?</div>.*?</article>)`,
        's'
    );
    
    updatedContent = updatedContent.replace(articlePattern, `$1${formattedDate}$2`);
});

// Script JavaScript completamente riscritto e semplificato
const finalJavaScript = `
                <script src="search-index.js"></script>
                <script>
                        document.addEventListener('DOMContentLoaded', function() {
                                console.log('Inizializzazione pagina news...');
                                ...
                                console.log('Dati articoli disponibili:', articlesData.length);
                                ...
                                function getArticleData(item) {
                                        ...
                                        console.log('Articolo:', filename, 'Dati trovati:', !!articleData);
                                        return articleData;
                                }
                                ...
                                function filterByYear(year) {
                                        ...
                                        if (year === 'all') {
                                                ...
                                                filteredItems = allNewsItems.filter(item => {
                                                        ...
                                                        const matches = articleData.year === targetYear;
                                                        console.log('Articolo:', articleData.title.substring(0, 30), 'Anno:', articleData.year, 'Target:', targetYear, 'Match:', matches);
                                                        return matches;
                                                });
                                        }
                                ...
                                console.log('Pagina news inizializzata correttamente');
                        });
                </script>
`;
...
updatedContent = updatedContent.replace(oldScriptRegex, finalJavaScript);

// Salva la pagina aggiornata
fs.writeFileSync(newsPagePath, updatedContent);

console.log('✅ Problemi risolti definitivamente!');
console.log('📅 Date sostituite direttamente nell\'HTML');
console.log('🎯 Filtro anno completamente riscritto e funzionante');
console.log('🔧 Debug logging aggiunto per verificare il funzionamento');
